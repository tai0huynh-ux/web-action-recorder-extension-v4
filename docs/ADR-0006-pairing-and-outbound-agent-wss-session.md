...
Managed Agent provisioning is idempotent with respect to an active device identity: a credential is generated from cryptographically secure random bytes only when no active credential exists. A repair or repeated provisioning request never rotates an existing credential implicitly. Reuse requires the current credential; missing or mismatched credentials are rejected without changing the persisted hash. TLS certificates and keys are never regenerated by repair because the Linux Agent must continue to trust the configured Controller CA.
