...
export function validateRuntimeProfile(value) {
  if (!isPlainObject(value) || !hasOnlyKeys(value, ['schemaVersion', 'wss'])) return invalid('Runtime profile has an invalid shape');
  ...
  return { ok: true, profile: deepFreeze({ schemaVersion: SCHEMA_VERSION, wss: { ...wss } }) };
}
...
function hasOnlyKeys(value, allowed) { return Object.keys(value).every((key) => allowed.includes(key)) && Object.keys(value).length === allowed.length; }
