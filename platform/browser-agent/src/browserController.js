...
const DEFAULT_NEW_TAB_URL = 'chrome://newtab/';
...
export class BrowserController {
  ...
  async captureRemoteFrame({ quality = 45, maxBytes = 500000 } = {}) {
    ...
    await this.ensureDefaultNewTab(page);
  ...
  async closeTab(targetId) {
    ...
    if (openPages.length <= 1) {
      await page.goto(DEFAULT_NEW_TAB_URL, { waitUntil: 'domcontentloaded' });
      this.activeTargetId = this.getTargetId(page);
  ...
  async ensureDefaultNewTab(page) {
    if (!page || page.isClosed() || page.url() !== 'about:blank') return false;
    await page.goto(DEFAULT_NEW_TAB_URL, { waitUntil: 'domcontentloaded' });
    return true;
  }
