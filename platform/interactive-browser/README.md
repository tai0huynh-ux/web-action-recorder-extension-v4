...
The Compose file joins an already-created external network named by `WAR_PILOT_NETWORK_NAME` (default `war-interactive-ipv6`); it never creates or changes the managed network. Sunshine 0.23.1 supports `address_family = ipv4|both`, not a specific bind address, so the IPv6-only pilot uses `both` and Docker publishes the stream only on the explicit private IPv4 `192.168.1.201` by default. Change that host binding with `WAR_PILOT_BIND_ADDRESS`; wildcard, loopback, public IPv4, and IPv6 host publications are rejected by the runtime contract. Sunshine advertises H.264-friendly 1280x720 at 30 fps with UPnP disabled. Typical ports are TCP 47984/47989/47990 and UDP 47998-48000. A direct public-IPv6 probe remains a required runtime acceptance check because the pinned Sunshine version listens on the IPv6 wildcard inside the container.
